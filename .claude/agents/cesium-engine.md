...
- **Lifecycle:** everything added must be removed. Collections added via `scene.primitives.add` are destroyed with the viewer, but feature modules that add/remove dynamically must `remove(primitive, true)` on teardown. React `useEffect` cleanups must be exact inverses of setup. Guard React 19 StrictMode double-invocation (existing `apps/web/src/app/GlobeView.tsx` pattern: ref-guard).
